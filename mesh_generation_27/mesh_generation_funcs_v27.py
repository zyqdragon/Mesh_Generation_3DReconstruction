...
def base_mesh_points(xseg,yseg):
    step_len=0.5
    len_x=step_len*xseg
    len_y=step_len*yseg
    points_coord_x=np.array([[0,0,0]])
    points_coord_y=np.array([[0,0,0]])
    # points_boundary=np.array([[0,0,0],[-10,1,0],[10,1,0]])
    for k in range(xseg+1):
        points_coord_x=np.append(points_coord_x,np.array([[ k*step_len, len_y,0]]),axis=0)
        points_coord_x=np.append(points_coord_x,np.array([[ k*step_len,-len_y,0]]),axis=0)
        points_coord_x=np.append(points_coord_x,np.array([[-k*step_len, len_y,0]]),axis=0)
        points_coord_x=np.append(points_coord_x,np.array([[-k*step_len,-len_y,0]]),axis=0)
    
    for j in range(yseg+1):
        points_coord_y=np.append(points_coord_y,np.array([[ len_x,  j*step_len,0]]),axis=0)
        points_coord_y=np.append(points_coord_y,np.array([[-len_x,  j*step_len,0]]),axis=0)
        points_coord_y=np.append(points_coord_y,np.array([[ len_x, -j*step_len,0]]),axis=0)
        points_coord_y=np.append(points_coord_y,np.array([[-len_x, -j*step_len,0]]),axis=0)
    
    print("----points_coord_x=",points_coord_x)
    print("----points_coord_y=",points_coord_y)
    lines_x=np.array([[0,0]]) 
    for k in range(int(np.floor(len(points_coord_x)/2))):
        # print("----k=",k)
        lines_x=np.append(lines_x,np.array([[k*2+1,k*2+2]]),axis=0)
    lines_y=np.array([[0,0]]) 
    for k in range(int(np.floor(len(points_coord_y)/2))):
        # print("----k=",k)
        lines_y=np.append(lines_y,np.array([[k*2+1,k*2+2]]),axis=0)
    print("-----lines_y=",lines_y)
    
    ###########################  start of boundary line of the point clouds #################################
    #设置点与点之间线段的颜色
    colors_x   = np.array([[0.2, 0.2, 0.2] for j in range(len(lines_x))])
    #创建Bbox候选框对象
    line_base_x = o3d.geometry.LineSet()
    #将八个顶点连接次序的信息转换成o3d可以使用的数据类型
    line_base_x.lines = o3d.utility.Vector2iVector(lines_x)
    #设置每条线段的颜色
    line_base_x.colors = o3d.utility.Vector3dVector(colors_x)
    #把八个顶点的空间信息转换成o3d可以使用的数据类型
    line_base_x.points = o3d.utility.Vector3dVector(points_coord_x)
    
    #设置点与点之间线段的颜色
    colors_y   = np.array([[0.2, 0.2, 0.2] for j in range(len(lines_y))])
    #创建Bbox候选框对象
    line_base_y = o3d.geometry.LineSet()
    #将八个顶点连接次序的信息转换成o3d可以使用的数据类型
    line_base_y.lines = o3d.utility.Vector2iVector(lines_y)
    #设置每条线段的颜色
    line_base_y.colors = o3d.utility.Vector3dVector(colors_y)
    #把八个顶点的空间信息转换成o3d可以使用的数据类型
    line_base_y.points = o3d.utility.Vector3dVector(points_coord_y)
    # line_base=0
    ###########################  end of boundary line of the point clouds ################################################
    return line_base_x,line_base_y
...
def mesh_face_generation(hseg,vseg,mesh_point_total,keypoints_num_total):
    ...
    face_keypoints_back=np.array([[0,0,0]])
    for k in range(0,(vseg-1)*2-1):  # [0,0,0] is connected to the box points  vseg=4; hseg=7;
        for j in range(0,hseg-1-1):
            if (flag_keypoint[k][j]+flag_keypoint[k][j+1]+flag_keypoint[k+1][j]+flag_keypoint[k+1][j+1])>19:
                ...
                face_keypoints_back=np.append(face_keypoints_back,np.array([[k*(hseg-1)+j+1,(k+1)*(hseg-1)+j,  k*(hseg-1)+j]]),axis=0)
                face_keypoints=np.append(face_keypoints,np.array([[(k+1)*(hseg-1)+j+1,k*(hseg-1)+j+1,  (k+1)*(hseg-1)+j]]),axis=0)
                face_keypoints_back=np.append(face_keypoints_back,np.array([[  (k+1)*(hseg-1)+j,k*(hseg-1)+j+1,(k+1)*(hseg-1)+j+1]]),axis=0)                
                ...
                if flag_keypoint[k+1][j]>4:
                    ...
                    face_keypoints_back=np.append(face_keypoints_back,np.array([[  k*(hseg-1)+j,(k+1)*(hseg-1)+j+1,(k+1)*(hseg-1)+j]]),axis=0)                    
                elif flag_keypoint[k][j+1]>4:
                    face_keypoints=np.append(face_keypoints,np.array([[  k*(hseg-1)+j,(k+1)*(hseg-1)+j+1,k*(hseg-1)+j+1]]),axis=0)
                    face_keypoints_back=np.append(face_keypoints_back,np.array([[  k*(hseg-1)+j+1,(k+1)*(hseg-1)+j+1,k*(hseg-1)+j]]),axis=0)
                ...
                if flag_keypoint[k][j]>4:
                    ...
                    face_keypoints_back=np.append(face_keypoints_back,np.array([[  k*(hseg-1)+j,(k+0)*(hseg-1)+j+1,(k+1)*(hseg-1)+j]]),axis=0)                    
                    ...
                    face_keypoints_back=np.append(face_keypoints_back,np.array([[  k*(hseg-1)+j+1,(k+1)*(hseg-1)+j+1,(k+1)*(hseg-1)+j]]),axis=0)
    
    # print("-----------face_keypoints=",face_keypoints)
    mesh_point_seq=mesh_point_total.reshape((vseg-1)*2*(hseg-1),3)

    #########################################start of counter clock wise face #################################
    max = np.amax(mesh_point_seq,axis=0)
    min = np.amin(mesh_point_seq,axis=0)
    # print("----mesh_point_seq[:3]=",mesh_point_seq)
    # print("----max=",max[0])
    colors = np.array([[0., 0., 0.8]]) 
    vert = [[0]*3]*len(mesh_point_seq)
    for k in range(len(mesh_point_seq)):
        # len_point=np.sqrt(np.square(mesh_point_seq[k][0])+np.square(mesh_point_seq[k][1]))
        vert[k]=[mesh_point_seq[k][0],mesh_point_seq[k][1],mesh_point_seq[k][2]]
        # colors=np.append(colors,len_point*np.array([[0,0,0.8]]),axis=0)
        cxs=(max-mesh_point_seq[k][0])*0.6/(max-min)+0.5
        colors=np.append(colors,cxs*np.array([[0,0,1.2]]),axis=0) 
        # colors=np.append(colors,np.array([[0,0.6,cxs*1.2]]),axis=0)    
    ...
    # print("-----vert1=",vert)
    # print("-----faces=",faces)
    # print("-----colors_a[1:]=",colors[1:])
    #########################################end of counter clock wise face #################################

    #########################################start of clock wise face #################################
    max = np.amax(mesh_point_seq,axis=0)
    min = np.amin(mesh_point_seq,axis=0)
    # print("----mesh_point_seq=",mesh_point_seq)
    # print("----max=",max[0])

    colors = np.array([[0., 0., 0.4]]) 
    vert = [[0]*3]*len(mesh_point_seq)
    for k in range(len(mesh_point_seq)):
        # len_point=np.sqrt(np.square(mesh_point_seq[k][0])+np.square(mesh_point_seq[k][1]))
        vert[k]=[mesh_point_seq[k][0],mesh_point_seq[k][1],mesh_point_seq[k][2]]
        # colors=np.append(colors,len_point*np.array([[0,0,0.8]]),axis=0)
        cxs=(max-mesh_point_seq[k][0])*0.6/(max-min)+0.5
        colors=np.append(colors,np.array([[0.0,0,0.4]]),axis=0)
        colors=np.append(colors,np.array([[0.0,0.4,0.4]]),axis=0)       
    faces_back = [[0]*3]*len(face_keypoints_back)
    for k in range(len(face_keypoints)):
        faces_back[k]=[face_keypoints_back[k][0],face_keypoints_back[k][1],face_keypoints_back[k][2]]
    m_back=o3d.geometry.TriangleMesh(o3d.utility.Vector3dVector(vert),
                            o3d.utility.Vector3iVector(faces_back))
    m_back.vertex_colors=o3d.utility.Vector3dVector(colors[1:])
    #########################################end of clock wise face #################################
    # print("-----vert2=",vert)
    # print("-----faces_back=",faces_back)
    # print("-----colors_b[1:]=",colors[1:])
    # m=face_keypoints
    return m,m_back