...
import numpy as np
from mesh_generation_funcs_v27 import *
...
def main():
    ...
    FOR2.translate((0.7,0.5, 1), relative=True)
    R = FOR2.get_rotation_matrix_from_xyz((0,np.pi,0))#绕y轴旋转90°
    FOR2.rotate(R)#旋转点位于x=20处，若不指定则默认为原始点云质心。
    FOR2.translate((+0.2,0, 0), relative=True)
    R = FOR2.get_rotation_matrix_from_xyz((0,0,np.pi/9))#绕y轴旋转90°
    FOR2.rotate(R)#旋转点位于x=20处，若不指定则默认为原始点云质心。
    FOR2.translate((0,-0.5, 0), relative=True)
    ...
    pcd2.translate((-0.1,1.2, 0), relative=True)
    ...
    m,m_back=mesh_face_generation(hseg,vseg,mesh_point_total,keypoints_num_total)
    xseg=10
    yseg=10
    line_base_x,line_base_y=base_mesh_points(xseg,yseg)

    line_base_x.translate((0.7,0.5, 1), relative=True)
    R = line_base_x.get_rotation_matrix_from_xyz((0,np.pi,0))#绕y轴旋转90°
    line_base_x.rotate(R)#旋转点位于x=20处，若不指定则默认为原始点云质心。
    line_base_x.translate((+0.2,0, 0), relative=True)
    R = line_base_x.get_rotation_matrix_from_xyz((0,0,np.pi/9))#绕y轴旋转90°
    line_base_x.rotate(R)#旋转点位于x=20处，若不指定则默认为原始点云质心。
    line_base_x.translate((0,-0.5, 0), relative=True)
    R = line_base_x.get_rotation_matrix_from_xyz((0,np.pi/2,0))#绕y轴旋转90°
    line_base_x.rotate(R)#旋转点位于x=20处，若不指定则默认为原始点云质心。



    line_base_y.translate((0.7,0.5, 1), relative=True)
    R = line_base_y.get_rotation_matrix_from_xyz((0,np.pi,0))#绕y轴旋转90°
    line_base_y.rotate(R)#旋转点位于x=20处，若不指定则默认为原始点云质心。
    line_base_y.translate((+0.2,0, 0), relative=True)
    R = line_base_y.get_rotation_matrix_from_xyz((0,0,np.pi/9))#绕y轴旋转90°
    line_base_y.rotate(R)#旋转点位于x=20处，若不指定则默认为原始点云质心。
    line_base_y.translate((0,-0.5, 0), relative=True)
    R = line_base_y.get_rotation_matrix_from_xyz((0,np.pi/2,0))#绕y轴旋转90°
    line_base_y.rotate(R)#旋转点位于x=20处，若不指定则默认为原始点云质心。
    ...
    mesh_pcd.paint_uniform_color([0,0,1]) #设置点云颜色为红色
    ...
    vis.add_geometry(line_base_x) 
    vis.add_geometry(line_base_y) 
